...
const ValuesPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      // Header letters animate with an upward slide and scale.
      ...
      // Subheadline letters: Only fade in without any slide or scale.
tl.fromTo(
  ...
  { 
    ...
    onStart: () => console.log("Subheadline animation started", performance.now()),
    onComplete: () => console.log("Subheadline animation completed", performance.now())
  },
  ">0"
);
  ...
  return (
    <>
                ...
                <header className="values-header mb-12">
                  ...
                  <p className="values-subheadline text-big font-normal text-gray-900">
                    {"Every decision we make involves benchmarking them with our values."
                      .split("")
                      .map((char, i) => (
                        <span key={i} className="inline-block animate-none will-change-transform">
                          {char === " " ? "\u00A0" : char}
                        </span>
                      ))}
                  </p>
      ...
      {/* Global override to cancel any default slide/transform animations on subheadline letters */}
      <style jsx global>{`
        .values-subheadline span {
